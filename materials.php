<?php 
    ...
    $sqlMaterialsStocksTable = "SELECT materials_tbl.ID, material_type_tbl.m_type, material_name_tbl.m_name, materials_tbl.m_name AS m_name_default, material_name_tbl.m_photo, MIN(materials_tbl.m_price) AS m_low_price, MAX(materials_tbl.m_price) AS m_high_price, COUNT(materials_tbl.m_name) AS m_name_count
                                FROM materials_tbl
                                INNER JOIN material_type_tbl ON materials_tbl.m_type = material_type_tbl.ID
                                INNER JOIN material_name_tbl ON materials_tbl.m_name = material_name_tbl.ID
                                WHERE materials_tbl.m_quality = 'Good' AND materials_tbl.m_status = 'Storage'
                                GROUP BY materials_tbl.m_name";
    $materialsStocks = $database->query($sqlMaterialsStocksTable) or die ($database->error);

    $sqlMaterialsUsedTable = "SELECT material_used_tbl.ID, material_used_tbl.m_code, material_name_tbl.m_name, material_name_tbl.m_photo, material_type_tbl.m_type, material_used_tbl.m_price, material_used_tbl.m_measurement, material_used_tbl.m_unit, material_used_tbl.m_date_used, material_used_tbl.p_code
                            FROM material_used_tbl
                            INNER JOIN material_name_tbl ON material_used_tbl.m_name = material_name_tbl.ID
                            INNER JOIN material_type_tbl ON material_used_tbl.m_type = material_type_tbl.ID";
    $materialsUsed = $database->query($sqlMaterialsUsedTable) or die ($database->error);
    ...
    if(isset($_POST['mDefective'])){
        ...
        $mSelectDescription = $_POST['selectDefectiveMaterialDescription'];
        $mRejected = date("Y-m-d H:i:s");

        if ($mSelectDescription == 'Others') {
            $mDescription = mysqli_real_escape_string($database, $_POST['inputDefectiveMaterialDescription']);
            $sqlMaterialsTable = "UPDATE `materials_tbl` SET `m_quality`='Bad', `m_rejected`='$mRejected', `m_description` = '$mDescription' WHERE `ID`='$mID'";
            $database->query($sqlMaterialsTable) or die ($database->error);
        }
        else {
            $sqlMaterialsTable = "UPDATE `materials_tbl` SET `m_quality`='Bad', `m_rejected`='$mRejected', `m_description` = '$mSelectDescription' WHERE `ID`='$mID'";
            $database->query($sqlMaterialsTable) or die ($database->error);
        }
                    ...
                    <div class="card">
                        <div class="card-header">
                            <ul class="nav nav-tabs card-header-tabs">
                                <li class="nav-item">
                                        <a class="nav-link text-dark border-top-success active" href="#mCatalog" data-bs-toggle="tab">Catalog</a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link text-dark border-top-success" href="#mStocks" data-bs-toggle="tab">Stocks</a>
                                </li>
                                <li class="nav-item ms-auto">
                                    <a class="nav-link text-dark border-top-secondary" href="#mUsed" data-bs-toggle="tab">Used</a>
                                </li>
                            </ul>
                        </div>
                        <div class="card-body">
                            <div class="tab-content">
                                ...
                                <!-- ==================== START - MATERIALS CATALOG TABLE ==================== -->

                                <div class="tab-pane active" id="mCatalog">
                                                                ...
                                                                <form action="" method="POST">
                                                                    ...
                                                                    <div class="modal-body">
                                                                        ...
                                                                        <div class="mb-2">
                                                                            ...
                                                                            <select class="form-select" id="selectDefectiveMaterialDescription" name="selectDefectiveMaterialDescription" onchange="selectDefectiveOption(this);" required>
                                                                        ...
                                                                        <div class="mb-2">
                                                                            <input type="text" id="test" class="test show">
                                                                        </div>
                                                                    </div>
                                                                    <div class="modal-footer">
                                ...
                                <!-- ==================== END - MATERIALS CATALOG TABLE ==================== -->

                                <!-- ==================== START - MATERIALS STOCKS TABLE ==================== -->

                                <div class="tab-pane" id="mStocks">
                                    <table class="datatable-asc-1 table table-hover responsive no-wrap w-100">
                                        ...
                                        <tbody>
                                            <?php while($fetch = $materialsStocks->fetch_array()){ $materialName = $fetch['m_name_default'];?>
                                                <tr>
                                                    <td class="align-middle"><?php echo "<img src='images/items/{$fetch['m_photo']}' class='rounded me-1' style='height: 50px; width: 50px; object-fit: cover;'> {$fetch['m_name']}";?></td>
                                                    <td class="align-middle"><?php echo $fetch['m_type']?></td>
                                                    <td class="align-middle">
                                                        <?php 
                                                            if ($fetch['m_low_price'] == $fetch['m_high_price']) {
                                                                echo number_format((float)$fetch['m_low_price'], 2, '.', ',');
                                                            }
                                                            else {
                                                                echo number_format((float)$fetch['m_low_price'], 2, '.', ',')." - ".number_format((float)$fetch['m_high_price'], 2, '.', ',');
                                                            }
                                                        ?>
                                                    </td>
                                                    <td class="align-middle">
                                                        <?php
                                                            ...
                                                            $materialsTablePieces = $database->query("SELECT * FROM materials_tbl WHERE m_name = '$materialName' AND m_status = 'Storage' AND m_quality = 'Good' AND m_unit = 'Piece'");
                                                            while ($row = $materialsTablePieces->fetch_assoc()) {
                                                                $materialsPiecesCount = $materialsPiecesCount+$row['m_remaining'];
                                                            }
                                                            echo $materialsPiecesCount;
                                                        ?>
                                                    </td>
                                                    <td class="align-middle">
                                                        <?php
                                                            $materialsKilogramsCount = 0;
                                                            $materialsTableKilograms = $database->query("SELECT * FROM materials_tbl WHERE m_name = '$materialName' AND m_status = 'Storage' AND m_quality = 'Good' AND m_unit = 'Kilogram'");
                                                            while ($row = $materialsTableKilograms->fetch_assoc()) {
                                                                $materialsKilogramsCount = $materialsKilogramsCount+$row['m_remaining'];
                                                            }
                                                            echo $materialsKilogramsCount;
                                                        ?>
                                                    </td>
                                                    <td class="align-middle">
                                                        <?php
                                                            $materialsMetersCount = 0;
                                                            $materialsTableMeters = $database->query("SELECT * FROM materials_tbl WHERE m_name = '$materialName' AND m_status = 'Storage' AND m_quality = 'Good' AND m_unit = 'Meter'");
                                                            while ($row = $materialsTableMeters->fetch_assoc()) {
                                                                $materialsMetersCount = $materialsMetersCount+$row['m_remaining'];
                                                            }
                                                            echo $materialsMetersCount;
                                                        ?>
                                                    </td>
                                                    <td class="align-middle">
                                                        <?php $mProgressBarCount = number_format((float)($fetch['m_name_count'] / $maxStorage) * 100, 1, '.', ',');?>
                                                        <div class='row d-flex align-items-center'>
                                                            <div class='col'>
                                                                <div class='progress' style='border: 1px solid 
                                                                    <?php
                                                                        if ($mProgressBarCount > 75) {
                                                                            echo "#198754";
                                                                        }
                                                                        else if ($mProgressBarCount > 50) {
                                                                            echo "#ffc107";
                                                                        }
                                                                        else if ($mProgressBarCount > 25) {
                                                                            echo "#fd7e14";
                                                                        }
                                                                        else if ($mProgressBarCount > 0) {
                                                                            echo "#dc3545";
                                                                        }
                                                                        else if ($mProgressBarCount == 0) {
                                                                            echo "#adb5bd";
                                                                    ...
                                                                    <div class='progress-bar 
                                                                        ...
                                                                        ?>' role='progressbar'style='width: <?php echo $mProgressBarCount;?>%' aria-valuemin='0' aria-valuemax='100'>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            <div class='col text-center'>
                                                                <label class='fw-bold'><?php echo $mProgressBarCount;?>%</label>
                                                            </div>
                                                        </div>
                                                    </td>
                                                </tr>
                                ...
                                <!-- ==================== END - MATERIALS STOCKS TABLE ==================== -->

                                <!-- ==================== START - MATERIALS USED TABLE ==================== -->

                                <div class="tab-pane" id="mUsed">
                                    <table class="datatable-desc-1 table table-hover responsive nowrap w-100">
                                        ...
                                        <tbody>
                                            <?php while($fetch = $materialsUsed->fetch_array()){ ?>
                                                <tr>
                                                    <td class="align-middle">
                                                        <?php echo "<img src='images/codes/{$fetch['m_code']}.png' class='rounded me-1' style='height: 50px; width: 50px; object-fit: cover;'> {$fetch['m_code']}";?>
                                                    </td>
                                                    <td class="align-middle"><?php 
                                                        echo "<img src='images/items/{$fetch['m_photo']}' class='rounded me-1' style='height: 50px; width: 50px; object-fit: cover;'> {$fetch['m_name']}";
                                                    ?></td>
                                                    <td class="align-middle"><?php echo $fetch['m_type']?></td>
                                                    <td class="align-middle"><?php echo number_format((float)$fetch['m_price'], 2, '.', ',');?></td>
                                                    <td class="align-middle"><?php echo $fetch['m_measurement'] . ' ' . $fetch['m_unit']?></td>
                                                    <td class="align-middle"><?php echo date('F j, Y, H:i a', strtotime($fetch['m_date_used']))?></td>  
